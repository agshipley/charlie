...
- **Shadow mode + operator review.** The adversary's output is informational, not editorial — it never changes what the brief says. Reader dispositions on adversary findings are collected (`/api/adversary/feedback`) and surfaced to the operator at **`/adversary/feedback`** (ADMIN_TOKEN-gated, read-only; also written to `data/adversary/feedback_export.md`). This feedback **never flows automatically back into the pipeline** — the operator reviews it and uses it to inform changes he makes by hand.
